...
const AboutBanner = (): JSX.Element => (
  ...
  >
    ...
    <div className="max-w-4xl mx-auto text-center space-y-8">
      ...
      >
        We started Baseline Core because we believed great software should feel effortless.
        Every decision we make — from architecture to interface — is driven by care for the people using it.
      </p>
    ...
    <div className="mt-24 w-full max-w-5xl mx-auto px-4 sm:px-6 grid grid-cols-1 md:grid-cols-3 gap-8">
      {[
        { title: 'Craft first', body: 'We sweat the details that most people overlook — because that\'s where the magic lives.' },
        { title: 'Human-centred', body: 'Every feature is designed around how people actually work, not how we think they should.' },
        { title: 'Open by default', body: 'We build in public, share our learnings, and believe the best ideas come from the community.' },
      ].map((value) => (
        <div
          key={value.title}
        ...
        >
          ...
          <p className="text-sm leading-relaxed" style={{ color: 'var(--warm-muted)' }}>{value.body}</p>
        </div>
      ))}
    </div>
  </main>
